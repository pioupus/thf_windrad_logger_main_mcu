...
/  https://github.com/blalor/FreeRTOS/blob/master/Demo/RX200_RX210-RSK_Renesas/RTOSDemo/Renesas-Files/hd44780.c
/-------------------------------------------------------------------------/
...
#define E1_HIGH() SET_DISP_E()   /* Set E(E1) high */
#define E1_LOW() CLEAR_DISP_E()  /* Set E(E1) low */
#define E2_HIGH()                /* Set E2 high (dual controller only) */
#define E2_LOW()                 /* Set E2 low (dual controller only) */
#define RS_HIGH() SET_DISP_RS()  /* Set RS high */
#define RS_LOW() CLEAR_DISP_RS() /* Set RS low */
#define OUT_DATA(d)                                                                                                                                  \
    boardDisplayWriteData(d >> 4) // LCD_DATA = ((LCD_DATA & 0xF0) | (d & 0x0F))	/* Output a byte d on the data bus (higher 4 bits of d in
                                  // 4-bit
                                  // mode)
                                  // */
...
#define LCD_ETIME_2_us 43 /* Execution time of other command and data write [us] */
#define LCD_DLF 2         /* Delay factor (>=2.0) */

const uint16_t DELAY_60ns = (60 / (TIMEBASE_SUBTICK_DURATION_us * 1000)) + 1;

const uint16_t DELAY_450ns = (450 / (TIMEBASE_SUBTICK_DURATION_us * 1000)) + 1;

const uint16_t DELAY_100us = (100 / TIMEBASE_SUBTICK_DURATION_us) + 1;

const uint16_t DELAY_ETIME2 = ((LCD_ETIME_2_us * LCD_DLF) / TIMEBASE_SUBTICK_DURATION_us) + 1;
...
static void lcd_write(uint8_t reg, /* b0:command(0)/data(1), b2..1:E1(2)/E2(1)/both(0)(don't care on single controller), b3:write high nibble
    ...
    timebase_delay_by_subticks(DELAY_60ns);
#if 0
    while (1) {
        timebase_delay_by_subticks(DELAY_100us);
...
#else
        E1_HIGH();
        timebase_delay_by_subticks(DELAY_450ns);
        E1_LOW();
#endif
        timebase_delay_by_subticks(DELAY_450ns);
        dat <<= 4;
    ...
    timebase_delay_by_subticks(DELAY_ETIME2); /* Always use timer */
}
...
void lcd_init(void) {
    ...
    vTaskDelay(40 / portTICK_RATE_MS); // delay_us(LCD_DT1); delay_us(40000);
    E1_LOW();
    ...
    //	DELAY_US(40000);
    lcd_write(8, 0x30);
    vTaskDelay(5 / portTICK_RATE_MS); // delay_us(4100);
    lcd_write(8, 0x30);
    timebase_delay_by_subticks(DELAY_100us);
    lcd_write(8, 0x30);
    ...
    lcd_write(0, 0x08);
    lcd_write(0, 0x01);
    vTaskDelay(3 / portTICK_RATE_MS); // delay_us(LCD_DT1);
    lcd_write(0, 0x06);
    lcd_write(0, 0x0C);
...
void lcd_putsn(const char *buffer, size_t n) {
...
void lcd_printf_at(int col, int row, const char *format, ...) {
    assert(row < _LCD_ROWS);
    assert(col < _LCD_COLS);
    lcd_locate(col, row);

    const size_t BUFF_SIZE = 64;
    char buffer[BUFF_SIZE + 1]; // forces null termination
    va_list args;
